...
def create_leaderboard_button_actionrow(is_first_page: bool, is_last_page: bool, random_id) -> dict:
    buttons = [
        ...
        manage_components.create_button(
            style=ButtonStyle.blue,
            label='🔄 Reload 🔄',
            custom_id="Reload" + random_id,
        ),
        manage_components.create_button(
            style=ButtonStyle.red,
            label='Delete',
            custom_id='Delete' + random_id
        ),
    ]
...
class General(commands.Cog):
    ...
    async def levels(self, ctx):
        ...
        while True:
            ...
            all_guild_accounts = self.my_database.get_all_from_guild(ctx.guild.id)
            if button_ctx.custom_id == 'Previous' + random_id:
                first_place -= 10
                last_place -= 10
                ...
                if first_place == 1:
                    ...
                    action_row = create_leaderboard_button_actionrow(False, False, random_id)
                try:
                    ...
                    print(e.text, e.code)
            elif button_ctx.custom_id == 'Next' + random_id:
                first_place += 10
                last_place += 10
                leaderboard = await self.get_leaderboard(all_guild_accounts, first_place, last_place)
                if len(all_guild_accounts) <= last_place:
                    action_row = create_leaderboard_button_actionrow(False, True, random_id)
                else:
                    action_row = create_leaderboard_button_actionrow(False, False, random_id)
                try:
                    await button_ctx.edit_origin(
            ...
            elif button_ctx.custom_id == 'Reload' + random_id:
                leaderboard = await self.get_leaderboard(all_guild_accounts, first_place, last_place)
                try:
                    await button_ctx.edit_origin(
                        content=f'**{ctx.guild.name}**\'s Leaderboard:\n```' + leaderboard + '```',
                    ...
                    print("Not found error!")
            elif button_ctx.custom_id == 'Delete' + random_id:
